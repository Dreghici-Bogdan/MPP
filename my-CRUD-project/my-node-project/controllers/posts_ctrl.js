...
export const createPost = async (req, res) => {
    const post = req.body;
    const { userid, difficulty, calories } = req.body;
    //show exact error
    if(!userid){
    ...
    if(!difficulty){
      return res.status(400).json({ error: 'Invalid data. Difficulty is required.' });
    }
    if (typeof difficulty !== 'string') {
      return res.status(400).json({ error: 'Invalid data. Difficulty must be a string.' });
    }
    if(!calories){
      return res.status(400).json({ error: 'Invalid data. Calories is required.' });
    }
  ...
  export const syncPosts = async (req, res) => {
    ...
    try {
      for (const post of posts) {
        ...
        if (existingPost) {
          if (post.id && !post.userid && !post.difficulty && !post.calories) {
...
export const updatePost = async (req, res) => {
    const { id } = req.params;
    const { difficulty, calories } = req.body;
    if (!difficulty || typeof difficulty !== 'string' || !calories ) {
      return res.status(400).json({ error: 'Invalid data. Difficulty (string) and calories (int) are required.' });
    }
